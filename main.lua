...
function Movement:_postMoveClear()
    local ok, err = self:clearHeadroom()
    if ok or err == "out_of_bounds" or not self.logger then
        return
    end
    self.logger:write("WARN", "Failed to clear headroom after move", err or "blocked")
end
...
local TUNNEL_LAYOUT_VERSION = 2

local function tunnelIsEarlier(a, b)
    local aOrder, bOrder = a.order or 99, b.order or 99
...
local function clampPositive(num, fallback)
    if not num or num < 1 then return fallback end
    return num
end

local function findSpanningChunk(chunks, z)
    if not chunks then return nil end
    for _, chunk in ipairs(chunks) do
        local span = chunk.span
        if span and z >= span.z0 and z <= span.z1 then
            return chunk
        end
    end
    return nil
...
function TunnelPlanner:ensureTunnels()
    local tunnels = self.store.data.tunnels or {}
    local version = self.store.data.tunnelsVersion or 0
    if version >= TUNNEL_LAYOUT_VERSION and next(tunnels) then
        return
    end
    ...
    local function nextId(prefix)
        ...
        return string.format("%s%04d", prefix, idCounter)
    end

    local chunkLength = clampPositive(self.config.chunkLength, 12)
    local branchSpacing = clampPositive(self.config.tunnelSpacing, 3)
    local zMax = self.config.boundingBox.z
    local branchStart = math.max(2, math.min(branchSpacing, zMax))
    local xMax = self.config.boundingBox.x
    local layerSpacing = clampPositive(self.config.layerSpacing, 3)

    local function addTunnel(entry)
        newTunnels[entry.id] = entry
        if entry.kind == "main" then
            walkwayIndex[entry.origin.y] = walkwayIndex[entry.origin.y] or {}
            table.insert(walkwayIndex[entry.origin.y], entry)
        end
    end

    for layer = 0, self.config.boundingBox.y - 1, layerSpacing do
        for zStart = 1, zMax, chunkLength do
            local length = math.min(chunkLength, zMax - zStart + 1)
            if length > 0 then
                local entry = {
                    id = nextId("M"),
                    kind = "main",
                    axis = "z",
                    direction = 1,
                    origin = { x = 0, y = layer, z = zStart },
                    length = length,
                    state = "idle",
                    progress = 0,
                    order = 0,
                    span = { z0 = zStart, z1 = zStart + length - 1 },
                }
                addTunnel(entry)
            end
        end

        for branchZ = branchStart, zMax, branchSpacing do
            local chunk = findSpanningChunk(walkwayIndex[layer], branchZ)
            if chunk then
                addTunnel({
                    id = nextId("B"),
                    kind = "branch",
    ...
    self.store.data.tunnels = newTunnels
    self.store.data.tunnelsVersion = TUNNEL_LAYOUT_VERSION

    local jobs = self.store.data.jobs or {}
    local function isValid(job)
        if job.type ~= "tunnel_mine" then return true end
        return newTunnels[job.payload and job.payload.tunnelId]
    end
    if jobs.active and not isValid(jobs.active) then
        jobs.active = nil
    end
    local filtered = {}
    for _, job in ipairs(jobs.pending or {}) do
        if isValid(job) then
            filtered[#filtered + 1] = job
        end
    end
    jobs.pending = filtered

    local tunnelTotal = 0
    for _ in pairs(newTunnels) do
        tunnelTotal = tunnelTotal + 1
    end
    self.logger:write("INFO", string.format("Initialized structured tunnel plan (%d entries)", tunnelTotal))
    self.store:save()
end
...
function TunnelPlanner:_requirementsMet(tunnel)
    if not tunnel.requires then
        return true
    end
    for _, depId in ipairs(tunnel.requires) do
        local dep = self.store.data.tunnels[depId]
        if not dep or dep.state ~= "done" then
            return false
        end
    end
    return true
...
function TunnelPlanner:claimNext(turtleId)
    ...
    for id, tunnel in pairs(self.store.data.tunnels) do
        if tunnel.state == "idle" and self:_requirementsMet(tunnel) then
...
function Worker:runTunnelJob(job)
    ...
    local axis = tunnel.axis or "z"
    local direction = tunnel.direction or 1
    local axisVec
    if axis == "x" then
        axisVec = { x = direction, y = 0, z = 0 }
    elseif axis == "z" then
        axisVec = { x = 0, y = 0, z = direction }
    else
        self.logger:write("WARN", "Unsupported tunnel axis", tostring(axis))
        return true, false
    end
    local target = {
        x = tunnel.origin.x + axisVec.x * tunnel.progress,
        y = tunnel.origin.y + axisVec.y * tunnel.progress,
        z = tunnel.origin.z + axisVec.z * tunnel.progress,
    }
    local ok, err = self.navigator:goTo(target)
    if not ok then
        self.logger:write("WARN", "Failed to reach tunnel origin", tunnel.id, err or "unknown")
        return false, true
    end
    local heading
    if axis == "x" then
        heading = direction == 1 and 1 or 3
    else
        heading = direction == 1 and 0 or 2
    end
    self.pose:faceAbsoluteDir(heading)
    ...
    ok, err = self.movement:forward()
    if not ok then
        self.logger:write("WARN", "Forward movement failed", err or "unknown")
        return false, true
    end
    self.scanner:scanNeighbors()
    tunnel.progress = tunnel.progress + 1
    self.store.data.metrics.mined = (self.store.data.metrics.mined or 0) + 1
